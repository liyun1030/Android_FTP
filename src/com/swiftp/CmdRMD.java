...
public class CmdRMD extends FtpCmd implements Runnable {
    ...
    public CmdRMD(FtpClientThread sessionThread, String input) {
        super(sessionThread, CmdRMD.class.toString());
        this.input = input;
    }

    public void run() {
        LogUtil.d("RMD executing");
        ...
        if (errString != null) {
            ...
            LogUtil.d("RMD failed: " + errString.trim());
        } else {
            sessionThread.writeString("250 Removed directory\r\n");
        }
        LogUtil.d("RMD finished");
    }
    ...
    protected boolean recursiveDelete(File toDelete) {
        ...
        if (toDelete.isDirectory()) {
            ...
            LogUtil.d("Recursively deleted: " + toDelete);
            return success && toDelete.delete();
        } else {
            LogUtil.d("RMD deleting file: " + toDelete);
