...
public class CmdPASV extends FtpCmd implements Runnable {
    ...
    public CmdPASV(FtpClientThread sessionThread, String input) {
        super(sessionThread, CmdPASV.class.toString());
    }

    public void run() {
        String cantOpen = "502 Couldn't open a port\r\n";
        LogUtil.d("PASV running");
        int port = sessionThread.onPasv();
        if (port == 0) {
            LogUtil.d("Couldn't open a port for PASV");
            sessionThread.writeString(cantOpen);
            return;
        }
        InetAddress addr = sessionThread.getLocalAddress();

        if (addr == null) {
            LogUtil.d("PASV IP string invalid");
            sessionThread.writeString(cantOpen);
            return;
        }
        LogUtil.d("PASV sending IP: " + addr.getHostAddress());
        if (port < 1) {
            LogUtil.d("PASV port number invalid");
            sessionThread.writeString(cantOpen);
            return;
        }
        ...
        LogUtil.d("PASV completed, sent: " + responseString);
    }
}
