...
public class CmdSIZE extends FtpCmd {
    ...
    public CmdSIZE(FtpClientThread sessionThread, String input) {
        super(sessionThread, CmdSIZE.class.toString());
        this.input = input;
    }

    public void run() {
        LogUtil.d("SIZE executing");
        ...
        mainblock: {
            ...
            if (!target.exists()) {
                ...
                try {
                    LogUtil.d("Failed getting size of: " + target.getCanonicalPath());
                } catch (IOException e) {
                }
                break mainblock;
            }
